...
%% copio parte de la fción loadEEG
load(sprintf('%s%s008-2014%sA%02d.mat',getdatasetpath(),filesep,filesep,subject))
% señal cruda: en dataX guarda las señales
dataX = data.X;
datatrial=data.trial;
%extrae las señales de hits crudas
trial=1;
%{
% rango trial 1
ini=data.trial(1);
...
for i=1:2
    ...
    %sx(i).x=baselineremover(sx(i).x,1,ws,ch,downsize); %lío índices
    % baselineremover a mano (restar la media)
    sx(i).x=sx(i).x-mean(sx(i).x);
    % estandarización
    sx(i).x=zscore(sx(i).x); % saqué el *2
end

% dibujo señales preprocesadas
s1p=sx(1).x;
s2p=sx(2).x;
